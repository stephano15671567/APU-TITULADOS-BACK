...
export const getProfesorIdInformante = async (req, res) => {
  const { rut } = req.params;
  console.log(rut)
  try {
    const connection = await createConnection();
    let [results] = await connection.query(
      "SELECT * FROM asignaciones_profesores WHERE asignaciones_profesores.alumno_RUT = ? AND asignaciones_profesores.rol = 'informante'",
      [rut]
    );
    if (results.length > 0) {
      results.sort((a, b) => new Date(b.fechaAsignacion) - new Date(a.fechaAsignacion));
